...
# ================= PAGE CONFIG =================
st.set_page_config(page_title="Cognitive Load Meter", layout="wide")

# ================= UI SPACING FIX =================
st.markdown("""
<style>
section[data-testid="stCameraInput"] { margin-bottom: -25px; }
img { margin-top: -10px; }
</style>
...
user_mode = st.sidebar.selectbox("Select User Type", ["Student", "Doctor", "Employee"])
if st.sidebar.button("Logout"):
    st.session_state.logged_in = False
    st.rerun()

# ================= MEDIAPIPE (CLOUD SAFE) =================
...
if img:
    ...
    if result.multi_face_landmarks:
        ...
        ear = (ear_calc(LEFT_EYE, lm, w, h) + ear_calc(RIGHT_EYE, lm, w, h)) / 2

        if ear < 0.20: score = 75
        elif ear < 0.25: score = 55

        for s, e in mp_face_mesh.FACEMESH_TESSELATION:
    ...
    status = "Normal"
    if score >= 65: status = "High Load"
    elif score >= 45: status = "Moderate Load"
    ...
    with pie_col:
        fig, ax = plt.subplots()
        ax.pie([calm, moderate, high], labels=["Calm","Moderate","High"], autopct="%1.0f%%")
        st.pyplot(fig)
